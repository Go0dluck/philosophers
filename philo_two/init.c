...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/18 20:37:06 by goodluck          #+#    #+#             */
/*   Updated: 2021/03/22 12:48:28 by ksharee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philo(t_all *all, char **agrv)
{
	...
	while (++i < all->size)
	{
		all->philo[i].all = all;
		all->philo[i].pos = i + 1;
		memset(sem_name, 'G', i + 1);
		sem_unlink(sem_name);
		if ((all->philo[i].protect =
			sem_open(sem_name, O_CREAT, 0666, 1)) == SEM_FAILED)
